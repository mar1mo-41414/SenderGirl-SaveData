...
- ペイロードのキー数は49個 (無印は46個)。無印の全フィールドが同じ
  ...
  (いずれも実機での意味確認はまだ。おそらく衣装/コスチューム関連):
  - `openClothesIds` — 15要素の配列
  - `selectClothes` — 単一の int (観測値: `-1`)
  - `nextTapItemAvailStatus` — 単一の int
- `tapItemLevel` の要素数が7個 (無印は4個)。5〜7番目のアイテム名は
  未確認。
- `facilitiesLevel` (11) / `powerupItemLevel` (11×4) / `teaItemLevel`
  (4) は無印と同じサイズ・同じ意味だった。
...
- K版限定のフィールドは「関西弁版限定」という専用タブに分離した
  (無印のセーブを開いている間はこのタブは無効化される)。意味が
  未検証なため、ラベルは仮のもので生の数値をそのまま編集する形に
  している。実機で値を変えて確認できたら、正式なラベル・選択肢に
  更新する予定。
