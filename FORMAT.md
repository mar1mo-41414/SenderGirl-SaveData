...
**【関西弁版】でも同一の境界 (127はOK、128以上でフリーズ) を実機で
確認済み** (2026-08-25)。totalClickCount の件とは異なり、こちらは
両版共通のバグと考えられる。おそらく無印・K版で共通の基盤コードを
使っているためだろう。
...
### ⚠️ `totalClickCount` (部屋タップ回数) は【関西弁版】でのみ編集するとフリーズする

上記の解放条件に使われているためか、`totalClickCount` を【関西弁版】
で直接編集すると実機でフリーズすることが確認された (2026-08-25)。
他のどの値との整合性が必要なのかは未検証。**タップ回数はいじらず、
`tapItemLevel` の状態を直接書き換えて解放する方が安全**
(この方法なら実際に確認済みで、タップ回数を合わせなくても問題なく
使えている)。

**無印ではこの問題は再現しない** ことも実機で確認済み (どれだけ
突拍子もない値を入れてもフリーズしなかった、とのこと)。おそらく
無印には存在しない、K版だけの解放条件チェックが原因と見られる。
そのため GUI では、無印を開いている間はこのフィールドを通常通り
編集可能にし、【関西弁版】を開いている間だけ無効化する
(currentCrystal のように両版共通で除外する扱いとは異なる)。
