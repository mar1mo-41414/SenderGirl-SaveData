...
無印の「ゆるヤミ彼女」(`com.Happygamer.SenderGirl`) に加えて、姉妹作
**【関西弁版】ゆるヤミ彼女と100万件のメッセージ**
(`com.Happygamer.SenderGirlK`) にも**実験的に対応**しています。

暗号化パスワードは両アプリで共通で、セーブデータのコンテナ形式・
共通フィールドの意味も同一であることを実データで確認済みです。開いた
セーブファイルがどちらの版かは自動判定し、画面右上にバッジで表示され
ます。

ただし K版だけに存在する3フィールド (`openClothesIds` /
`selectClothes` / `nextTapItemAvailStatus`。おそらく衣装関連) は
**実機での意味確認がまだ** で、GUIの「関西弁版限定」タブでは仮の
ラベルのまま生の数値を編集する形になっています。詳細は
